...
| `determinismRuns` | integer | how many consecutive red runs observed | ≥ 1 to pass TRDA `deterministic` |
...
| `transitions[]` | table | append-only audit log; each row `{from, to, at:ISO8601, actor:human\|agent, reason:string\|null}` |
...
- `protocols/loom-converge.interaction.md` (CREATE — stub-only at this phase; body lands in Phase 3)
...
- [ ] `protocols/grilling.md` and `protocols/loom-converge.interaction.md` exist as stubs with at minimum a one-paragraph "what this protocol covers" body; full content lands in Phases 3 / 8.
...
- [ ] `/loom-which` skill ships with a decision tree of ≥6 branch nodes and explicit one-question-at-a-time behavior; first-question recommendation is presented per `protocols/grilling.md` discipline.
...
### Phase 2 — Wave 2: Phase B — tight-red feedback loop (HEADLINE BEHAVIOURAL CHANGE)

**Agent:** implementer-agent
**Objective:** Replace the current hypothesise-first bug/convergence flow with a tight-red feedback-loop discipline: `loom-bugfix` Phase-1 gate, `loom-converge` Phase-0 loop construction, `feedback-loop` model-invoked skill with 10-rung ladder + TRDA, `tdd-coach` horizontal-slice anti-pattern + no-silent-regression rule. The headline behavioural change in F-18.
**Dependencies:** Phase 0, Phase 1
**File Ownership:**
- `agents/loom-bugfix-driver.md` (MODIFY)
- `agents/bugfix-analyst-agent.md` (MODIFY)
- `agents/debug-investigator-agent.md` (MODIFY)
- `agents/loom-converge-driver.md` (MODIFY) *or equivalent driver entry — implementer resolves by inspection*
- `agents/converge-stage-teammate.md` (MODIFY)
- `agents/tdd-coach.md` (MODIFY) <!-- Applied: P-06 — filename resolved; confirmed present in worktree -->
...
- `skills/feedback-loop/library.yaml` registration (MODIFY `skills/library.yaml`)
- `protocols/loom-converge.interaction.md` (MODIFY — replace stub with full content)
- `tests/agents/loom-bugfix-gate.test.ts` (CREATE)
- `tests/agents/loom-converge-loop-construction.test.ts` (CREATE)
- `tests/agents/tdd-coach-anti-pattern.test.ts` (CREATE)
- `tests/regressions/stuck-at-loop-construction.test.ts` (CREATE)
- `tests/regressions/linked-loops-lint-typecheck.test.ts` (CREATE)
- `tests/skills/feedback-loop.test.ts` (CREATE)

#### Deliverables

| File | Action | Owner hint |
|------|--------|------------|
| `agents/loom-bugfix-driver.md` | Modify | implementer-1 |
| `agents/bugfix-analyst-agent.md` | Modify | implementer-1 |
| `agents/debug-investigator-agent.md` | Modify | implementer-1 |
| `agents/loom-converge-driver.md` | Modify | implementer-1 |
| `agents/converge-stage-teammate.md` | Modify | implementer-1 |
| `agents/tdd-coach.md` | Modify | implementer-1 |
| `commands/loom-bugfix.md` | Modify | implementer-1 |
| `commands/loom-converge.md` | Modify | implementer-1 |
| `skills/feedback-loop/SKILL.md` | Create | implementer-1 |
| `skills/library.yaml` | Modify | implementer-1 |
| `protocols/loom-converge.interaction.md` | Modify (replace stub) | implementer-1 |
| `tests/agents/loom-bugfix-gate.test.ts` | Create | implementer-1 |
| `tests/agents/loom-converge-loop-construction.test.ts` | Create | implementer-1 |
| `tests/agents/tdd-coach-anti-pattern.test.ts` | Create | implementer-1 |
| `tests/regressions/stuck-at-loop-construction.test.ts` | Create | implementer-1 |
| `tests/regressions/linked-loops-lint-typecheck.test.ts` | Create | implementer-1 |
| `tests/skills/feedback-loop.test.ts` | Create | implementer-1 |
...
- [ ] Exhausted 10-rung ladder produces named `stuck-at-loop-construction` state with HITL escalation guidance (not silent block) — verified by `tests/regressions/stuck-at-loop-construction.test.ts` which intentionally hits the dead-end.
...
- [ ] `skills/feedback-loop/SKILL.md` ships with the full 10-rung ladder (failing test → curl → CLI+fixture diff → headless browser → trace replay → throwaway harness → fuzz → bisection → differential → HITL bash), TRDA gate definition, and tighten-the-loop heuristics. Leading words `tight` and `red` are present verbatim.
- [ ] `tdd-coach` MD frontmatter and body incorporate the horizontal-slice anti-pattern framing verbatim and add the no-silent-regression-during-refactor rule (test count must not decrease during a refactor step) — verified by a fixture test that asserts the canonical sentences appear in the agent body.
- [ ] <!-- Applied: P-08 --> Phase 2 ships AT LEAST 3 edge-case regression tests beyond the gate-presence test: (a) re-retire of an already-retired loop exits `LOOP_IMMUTABLE`, (b) read of a retired loop's state succeeds (queryable-after-retire), (c) `--override-loop-gate` with an empty reason fails validation. Each MUST be a discrete `it()` block in the named test files; coverage report records all three.
- [ ] `bunx vitest run tests/agents tests/regressions tests/skills/feedback-loop.test.ts` exits 0.
...
- `agents/loom-plan-create.md` (MODIFY) *implementer resolves the canonical filename*
- `agents/loom-plan-materialize.md` (MODIFY)
- `agents/parallelization-agent.md` (MODIFY)
- `agents/phasing-agent.md` (MODIFY)
<!-- Applied: P-03 — explicit disjointness with Phase 4 -->
**Disjoint from Phase 4 (asserted):** Phase 3 owns ONLY the four planning agents above (`loom-plan-create`, `loom-plan-materialize`, `parallelization-agent`, `phasing-agent`) under `agents/`. Phase 3 does NOT touch `agents/roadmap-converge-reviewer.md`, `agents/code-reviewers/**`, or `commands/loom-note.md` / `commands/loom-do.md` — those are exclusively Phase 4's. The wave validator MUST flag any overlap.
...
| `agents/loom-plan-create.md` | Modify | implementer-c |
| `agents/loom-plan-materialize.md` | Modify | implementer-c |
...
- `agents/code-reviewers/*.md` (MODIFY — add ADR cross-check section; implementer enumerates which reviewer files exist)
<!-- Applied: P-03 — explicit disjointness with Phase 3 -->
**Disjoint from Phase 3 (asserted):** Phase 4 owns `agents/roadmap-converge-reviewer.md` AND every `agents/code-reviewers/*.md` reviewer (implementer enumerates: typically `agents/code-reviewers/correctness.md`, `agents/code-reviewers/security.md`, `agents/code-reviewers/style.md` — wave-0 must materialize the exact list and pin it to this phase). Phase 4 does NOT touch `agents/loom-plan-*.md`, `agents/parallelization-agent.md`, or `agents/phasing-agent.md` — those are exclusively Phase 3's. The wave validator MUST flag any overlap.
...
| `agents/code-reviewers/*.md` (per-reviewer; subject to inventory) | Modify | implementer-d |
...
- [ ] `needs-info → wontfix` after 30 days of no response — verified by `tests/scripts/state-machine.test.ts` with a date-mocked fixture.
...
- [ ] `bunx vitest run tests/commands/loom-note-triage.test.ts tests/commands/loom-do-redundancy.test.ts tests/scripts/out-of-scope-suppress.test.ts tests/agents/roadmap-converge-reviewer-oos.test.ts tests/agents/code-reviewer-adr-conflict.test.ts` exits 0.
...
### Phase 5 — Wave 4: Phase E — session + presentation polish

**Agent:** implementer-agent
**Objective:** Land the final polish layer — `loom-pause` handoff hygiene (edit-in-place), `grilling.md` full content with 12-question cap, conditional HTML report mode across more surfaces, skill autoload audit with deprecation notices, final sediment sweep.
...
- `agents/architecture-review-agent.md` (MODIFY) <!-- Applied: P-10 — host the CT-07 vocab-collision qa-review pass -->
...
| File | Action | Owner hint |
|------|--------|------------|
| `commands/loom-pause.md` | Modify | implementer-e |
| `agents/loom-pause-handoff-author.md` | Modify | implementer-e |
| `scripts/loom-pause/*.ts` | Create | implementer-e |
| `protocols/grilling.md` | Modify | implementer-e |
| `commands/loom-status.md` | Modify | implementer-e |
| `commands/loom-roadmap/status.md` | Modify | implementer-e |
| `scripts/html-renderer/*.ts` | Create | implementer-e |
| `scripts/skill-autoload-audit/*.ts` | Create | implementer-e |
| `scripts/sediment-sweep/no-op-test.ts` | Create | implementer-e |
| `commands/loom-doctor.md` | Modify | implementer-e |
| `agents/architecture-review-agent.md` | Modify | implementer-e |
| `tests/agents/architecture-review-vocab-collision.test.ts` | Create | implementer-e |
| `tests/commands/loom-pause-handoff.test.ts` | Create | implementer-e |
| `tests/protocols/grilling-12cap.test.ts` | Create | implementer-e |
| `tests/commands/loom-status-html.test.ts` | Create | implementer-e |
| `tests/commands/loom-roadmap-status-html.test.ts` | Create | implementer-e |
| `tests/scripts/skill-autoload-audit.test.ts` | Create | implementer-e |
| `tests/scripts/sediment-sweep.test.ts` | Create | implementer-e |
| `tests/regressions/handoff-shim.test.ts` | Create | implementer-e |
...
- [ ] `protocols/grilling.md` documents: one question at a time, recommend an answer, walk every branch, prefer codebase exploration over asking, cap of 12 questions per session, progress indicator format, and `/skip` escape.
...
- [ ] Sediment sweep runs the `writing-great-skills` no-op test sentence-by-sentence across all Loom `SKILL.md` files; mid-flight pass after Phase 2 (recorded as a baseline) plus final pass at Phase 5 end retires ≥20% of body lines net.
- [ ] <!-- Applied: P-10, CG-002 --> `architecture-review-agent` body is updated to run a vocabulary-collision pass: a diff fixture exercises the agent and asserts it (a) flags any mention of `phase/wave/deliverable` used as a synonym for `Module/Seam/Adapter` in the same paragraph, and (b) emits a finding citing `protocols/codebase-design.md` Section 0. Verified by `tests/agents/architecture-review-vocab-collision.test.ts`.
- [ ] `bunx vitest run tests/commands tests/protocols tests/scripts tests/regressions/handoff-shim.test.ts` exits 0.

#### Convergence Targets

- `loom-pause` fixture run: handoff file lives in `$TMPDIR`, contains the `suggestedSkills[]` section, contains zero duplicated PRD text.
...
- Sediment sweep report shows ≥20% body-line retirement net across the SKILL.md corpus.
...
```toon
id: S-07
title: architecture-review-agent flags Module/phase vocabulary collisions
given[2]: A fixture diff that mixes "Module" and "phase" as synonyms in the same paragraph, architecture-review-agent body cites protocols/codebase-design.md Section 0
when: architecture-review-agent reviews the fixture diff
whenTriggerType: actor-action
...
- [ ] **CT-01:** A new `loom-bugfix` run on a hard bug halts at Phase 1 until `loop.toon` exists with `verifiedRed: true` and TRDA gates passed; no hypothesis work before then. Gate applies to autoconverge AND default paths. **Covered by Phase 2 scenarios S-01, S-02.**
- [ ] **CT-02:** An exhausted 10-rung ladder produces a named `stuck-at-loop-construction` state with HITL escalation guidance — verified by a regression test that hits the dead-end intentionally. **Covered by Phase 2 scenario S-03.**
- [ ] **CT-03:** A `loom-converge` run binds each iteration to a single `loopId` and command; lint/typecheck failures spawn their own loops, tracked via `linkedLoops[]`, and surface in `convergence-state.toon`. **Covered by Phase 2 scenarios S-04, S-05.**
...
- [ ] **CT-07:** A vocabulary collision test: agent output for an architecture review uses `Module/Seam/Adapter` terms consistently and never mixes them with `phase/wave/deliverable` for the same concept. **Covered by Phase 0 scenario S-02 (table parser) + Phase 5 scenario S-07 (architecture-review-agent vocab-collision pass).** <!-- Applied: P-10 -->
- [ ] **CT-08:** The `/loom-skill create` wizard's output for a new skill satisfies the no-op test sentence-by-sentence; sediment sweep across existing skills retires ≥20% of body lines. **Covered by Phase 5 scenario S-06.**
...
|-----------|--------|-------------|
| M-08-Phase-A | Phase 0, Phase 1 | All Phase-A foundations land; pre-F-18 fixtures parse unchanged. |
| M-08-Phase-B | Phase 2 | Tight-red loop discipline replaces hypothesise-first; CT-01, CT-02, CT-03 all green. |
...
| M-08-Phase-E | Phase 5 | Polish; CT-07 + CT-08 green. |
