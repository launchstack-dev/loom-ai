...
**Objective:** Define all TypeScript interfaces for v4 catalog migration, extract the `skill-router.ts` helper module, produce the catalog-v4-exports contract artifact, audit `install-state-migrator.ts`, and stub test fixture files.
**Dependencies:** None
...
**File Ownership:** hooks/lib/library-catalog-migrator.ts (interface additions only), hooks/lib/skill-router.ts (new), test-fixtures/library-catalog-migration/**, .plan-execution/contracts/**
...
| `.plan-execution/stage-context/phase-4.toon` | Create — record test file created, fixture paths, key ACs confirmed (path construction, deprecation flag, invalid path rejection), current-version state, any warnings emitted | implementer-agent |
...
<!-- P3-07: phase-4.toon stage-context write per execution-conventions requirement -->
- [ ] `.plan-execution/stage-context/phase-4.toon` exists and records test file created and key ACs confirmed
...
| File | Action | Owner hint |
|------|--------|------------|
| `commands/loom-skill.md` | Create — `/loom-skill create` wizard implementing the interview flow: (1) ask for skill name (slug validation); (2) ask for description; (3) ask whether file-triggered (offer glob examples) or description-activated (omit triggers); (4) **before writing any files**: confirm the choice: "Generate {choice}-activated skill? [y/N]" — user may answer N to go back and revise (N-03 step-correction escape); (5) generate `SKILL.md` with frontmatter (`name:`, `description:`, optional `triggers:`); (6) write to `skills/<name>/SKILL.md`; (7) check whether `library.skills:` in `skills/library.yaml` already has an entry for this name — if yes, print warning and offer overwrite/abort (idempotency gate); (8) append entry under `library.skills:` in `skills/library.yaml`; (9) ask whether to add to a kit's `includes:` (offer kit name input); (10) print explicit install instruction: **"Run `/loom-library use <name>` to install the skill to `~/.claude/skills/`"**; (11) print restart notice: **"Then restart your Claude Code session for trigger activation to take effect."** — restart notice appears AFTER the install instruction | implementer-agent |
...
**Agent:** implementer-agent
**Objective:** Update the content-classification heuristic in `commands/loom-library.md` to correctly distinguish between native skills (triggers: frontmatter presence) and protocol files (inter-agent schema markers) in the post-v4 catalog. Heuristic prioritizes `triggers:` frontmatter presence over filename pattern.
...
| File | Action | Owner hint |
|------|--------|------------|
| `commands/loom-library.md` | Modify — update `/loom-library add` classification heuristic: (1) **`triggers:` frontmatter present (regardless of filename)** → `type: skill` (primary signal; N-07); (2) SKILL.md filename without `triggers:` → **ambiguous** (not auto-classified as skill); (3) presence of inter-agent schema markers (AgentResult, state.toon, etc.) → `type: protocol`; (4) `$ARGUMENTS` → `type: prompt`; (5) agent-style markers → `type: agent`; (6) ambiguous → display the following prompt: `"The source file's type is ambiguous. Choose:\n  [1] skill    — activates automatically on matching file patterns via Claude Code (SKILL.md format)\n  [2] protocol — inter-agent message schema used by Loom orchestration\n  [q] abort\n\nSelection:"` | implementer-agent |
