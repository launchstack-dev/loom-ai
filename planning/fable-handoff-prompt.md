...
Read `planning/ROADMAP-fable-readiness.md` first. It is the authoritative plan: vision, layer membership table, profile spec, C-01..C-08 locked decisions, milestones M-1..M-4, and out-of-scope list. Honor the locked decisions; if you believe one is wrong, stop and surface the disagreement with evidence rather than silently deviating. Also read `CLAUDE.md`, `CONTEXT.md`, `planning/ct6-to-loom-port-plan.md` (reference for the Track A guarantees the upstream maintainer owns per C-08 — you integrate them, you do not implement them), and skim `protocols/execution-conventions.md` before touching code.
...
2. **M-2** — port the convergence loop (all three modes) and wave execution to executable Workflow/TS scripts with model-chosen shape inside script-enforced caps (C-07); integrate the upstream maintainer's CT6 Track A ports per C-08 (wire each ported gate through `discipline.ts` as it lands; do NOT implement the ports yourself — the maintainer owns them); implement goal-backward verification as a convergence criterion (that one is ours); freeze and label the markdown drivers as `fallback`.
...
**Excluded — do not touch:** re-implementing the CT6 Track A guarantees (acceptance re-validation stop-gate, map-freshness gate, adversarial-reviewer, coverage matrix) — the upstream maintainer's port owns those; your job is layer slots + discipline-seam wiring when they land. Also excluded: TOON→JSON migration of existing artifacts; deleting markdown fallback drivers; install.sh / checksums / marketplace packaging semantics; new user-facing slash commands; multi-model routing redesign; wiki/change-proposal ceremony changes; anything in the user's `~/.claude` outside this repo's install targets.

**Upstream coordination:** before touching the Stop hook, `hooks/lib/`, or `orchestration.toml`, check `git log`/open PRs on upstream `main` for the maintainer's CT6 port activity in those files, and rebase onto latest `main` at every milestone start. If the maintainer's work conflicts with a locked decision, stop and surface it.
