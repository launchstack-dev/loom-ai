...
| `reviewerAgent` | string | no | Reviewer attribution (locked W-03). Populated by the plan-review harness with a reviewer id from the run's configured panel — the DEFAULT panel is the 6 canonical reviewers (`feature-coverage-reviewer-agent`, `strategy-reviewer-agent`, `ux-reviewer-agent`, `phasing-reviewer-agent`, `parallelization-reviewer-agent`, `agentic-workflow-reviewer-agent`), overridable per C-07 via a `reviewers[N]{reviewerAgent,agentFile}:` array in converge.config — OR the synthetic identifier `validation-rules-stages-1-4` when the row is written by the `/loom-plan create --autoconverge` Step 5.5 post-converge validation gate (NOT a real agent — names the rule set that produced the finding). Optional for non-harness contexts (e.g., when a single-agent harness writes findings directly). |
...
10. **`reviewerAgent` membership (when populated).** When the harness is the plan-review harness, `reviewerAgent` MUST be a member of the run's configured reviewer panel (default: the 6 canonical reviewer agent names; overridable per roadmap C-07 via `reviewers[]` in converge.config — non-canonical reviewers derive their `dimension` from the agent name). The synthetic identifier `validation-rules-stages-1-4` is ALSO permitted on rows whose `dimension == structural-validation` — these rows are written by the `/loom-plan create --autoconverge` Step 5.5 wrapper, not by a reviewer agent, but co-mingle in the same `findings.toon` so the integrator can resolve them through the existing convergence loop. For other harnesses, `reviewerAgent` is free-form or omitted.
