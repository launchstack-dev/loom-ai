...
> **Note:** The upstream schema is owned by Anthropic and documented at <https://code.claude.com/docs/en/plugins-reference>. This file is a **Loom-side snapshot** used for static validation in `test/plugin-manifest.test.ts` and as the contract for `wave-1-manifest-agent`. The JSON Schema mirror lives at `protocols/upstream/plugin.schema.json` and is refreshed by `scripts/refresh-upstream-schemas.sh` (CI workflow). On upstream drift, this file and `hook-manifest.schema.md` are the only artifacts that need updating.
