...
2b. **Canonicalize `checkoutRoot` to the main checkout.** Run `git -C "${checkoutRoot}" rev-parse --git-common-dir`. If the resolved common dir's parent directory differs from `checkoutRoot`, the catalog symlink is anchored inside a **linked git worktree**. Symlinks written against a worktree die the moment the worktree is removed — a real incident took all 26 installed Loom skills dark when `.worktrees/exploregstack` was deleted. Re-anchor: set `checkoutRoot` to the common dir's parent (the main checkout), print `warning: catalog symlink points into a worktree; re-anchoring to {mainRoot}`, and treat every managed symlink whose target is under the worktree path as `[rotate]` in the Branch B plan (rewrite it to the main-checkout equivalent). NEVER create a new symlink whose target path contains `/.worktrees/` or `/.claude/worktrees/` — abort that item with `SOURCE_VALIDATION_ERROR` instead. (Gstack remediation — dangling-worktree incident, 2026-07.)
