...
1. **Review agent** — runs during /loom-review-code (e.g., compliance checker, style enforcer)
2. **Execution agent** — runs during /loom-execute-plan (e.g., migration writer, seed data generator)
3. **Testing agent** — runs during /loom-test-plan (e.g., contract test writer, load test generator)
4. **Planning agent** — runs during /loom-review-plan (e.g., domain validator, cost estimator)
...
You are a {role} that reviews code changes for {domain concern}. You are spawned by `/loom-review-code` as part of a parallel review fan-out.
...
You are a {role} that {action} during plan execution. You are spawned by `/loom-execute-plan` at the `{phase}` phase.
...
You are a {role} that {action} during the test pipeline. You are spawned by `/loom-test-plan` at the `{phase}` phase.
...
You are a {role} that reviews project plans for {domain concern}. You are spawned by `/loom-review-plan` alongside other planning reviewers.
...
- **Domain mismatch** — don't create an agent that duplicates what a built-in reviewer already does. Check what `/loom-review-code` already runs.
