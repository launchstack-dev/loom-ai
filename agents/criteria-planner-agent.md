...
11. **Emit the coverage matrix (C-08).** Alongside criteria-plan.toon, write `.plan-execution/coverage-matrix.toon` per `protocols/coverage-matrix.schema.md`: one row per acceptance criterion (`source: criteria`) AND one row per phase-promise deliverable found in `.plan-execution/phase-*-promise.toon` (`source: phase-promise`, per `protocols/phase-promise.schema.md`). Rows start `uncovered` unless a passing testRef already exists. The convergence engine treats every non-covered row as blocking — this is how goal-backward verification ("did we deliver what the phase promised?") enters the loop.
12. **Include registered adversarial reviewers.** When orchestration.toml registers an `adversary-agent` under `[[review.agents]]`, add it to the reviewers table (blocking) — its refutations are soft criteria like any other reviewer's findings.
